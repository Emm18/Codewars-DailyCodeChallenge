...
//will be used, using an array/list to compute the number of the survivor may be too slow; you may assume that both n and k 
//will always be >=1.
...
function josephusSurvivor(n, k) {
  ...
  let numLength = newNum.length - 1;
  let indexCtr = 0;
  let removerCtr = 1;

  while (newNum.length > 1) {
    if (removerCtr == k) {
      newNum.splice(indexCtr, 1);
      removerCtr = 1;
      numLength -= 1;
    } else {
      removerCtr++;
