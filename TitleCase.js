...
function titleCase(title, minorWords) {
  ...
  let toLowerWords = minorWords ? minorWords.toLowerCase().split(' ') : [];
  
  for(let i = 0; i <= titleArr.length - 1; i++){
    if(i == 0){
      titleArr[i] = toCapitalize(titleArr[i]);
    } else {
      if(toLowerWords.includes(titleArr[i].toLowerCase())){
        let temp = titleArr[i].toLowerCase();
        titleArr[i] = temp;
      } else {
        titleArr[i] = toCapitalize(titleArr[i]);
      };
...
function toCapitalize(str){
  let temp1 = str.split('');
  let temp2 = temp1.splice(0,1)[0].toUpperCase();
  if(temp1){
    temp1 = temp1.join('').toLowerCase();
  };
  return temp2 + temp1;
};
