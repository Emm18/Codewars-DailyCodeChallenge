...
//They are dealt the same number of cards. They both flip the card on the top of their deck. Whoever has a card with higher value wins 
//the round and gets one point (if the cards are of the same value, neither of them gets a point). After this, the two cards are discarded 
//and they flip another card from the top of their deck. They do this until they have no cards left.
...
function winner(deckSteve, deckJosh) {
  let ranking = ['2','3','4','5','6','7','8','9','T','J','Q','K','A'];
  let s = 0;
  let j = 0;
  
  for(let i = 0; i <= deckSteve.length - 1; i++){
    let steve = ranking.indexOf(deckSteve[i]);
    let josh =  ranking.indexOf(deckJosh[i]);
    if(steve > josh){
      s++;
    };
  ...
  return s == j ? "Tie" : s > j ? `Steve wins ${s} to ${j}` : `Josh wins ${j} to ${s}`;
};
