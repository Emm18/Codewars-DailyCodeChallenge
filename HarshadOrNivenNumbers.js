...
var Harshad = ( function() {
  ...
  const addDigits = function (number) {
    let total = 0;
    number.toString().split('').map(x => total += parseInt(x));
    return total;
  };
  
  return {
    isValid: function( number ) {
      return number % addDigits(number) == 0 ? true : false;
    },
    getNext: function( number ) {
      let total = 0;
      number++;
      
      while(!(number % addDigits(number) == 0)){
        total = 0;
        number++;
      };
      return number;
    ...
    getSerie: function( count, start ) {
      let result = [];
      let number = start ? start + 1 : 1;
      while(result.length < count){
        if(number % addDigits(number) == 0){
          result.push(number);
        };
        number++;
      };
