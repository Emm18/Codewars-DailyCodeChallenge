...
function wheatFromChaff(values) {
    ...
    values.filter((x, index) => {
        if(x < 0){
          negativeIndex.push([x, index]);
        }
    });
    negativeIndex.reverse();
    
    let negativeCtr = negativeIndex.length;
    
    while(start < negativeCtr){
      let temp = values[start];
      if(temp > 0){
        let temp2 = negativeIndex.splice(0,1)[0];
        let tempVal = temp2[0];
        let tempIndex = temp2[1];
        values[start] = tempVal;
