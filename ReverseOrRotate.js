...
function revrot(str, sz) {
    ...
    let chunksLength = Math.floor(newStr.length / sz);
    
    if(str == '' || sz > newStr.length || sz == 0){
      return '';
    }

    while(chunksLength > 0){
      chunks.push(newStr.splice(0,sz))
      chunksLength--;
    }
    
    for(let i = 0; i <= chunks.length - 1; i++){
      let cubed = 0;
      chunks[i].map(val => cubed += Math.pow(val,3));
      
      if(cubed % 2 == 0){
        chunks[i] = chunks[i].reverse();
      } else {
        let temp = chunks[i].splice(0,1)[0];
        chunks[i].push(temp)
      }
