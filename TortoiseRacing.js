...
function race(v1, v2, g) {
  ...
  time.push(getHour(getDecimal));
  time.push(getMinutes(getDecimal));
 
  time.push(getSeconds(getDecimal));
  if(getSeconds(getDecimal) == 60) {
    time[1] += 1;
    time[2] = 0;
  }
...
function getSeconds(t) {
  ...
  let z = ((y * 60) - Math.floor((y * 60))) * 100;
