...
function encode(text) {
  ...
  newText.map(str => {
    ("00000000"+str.charCodeAt(0).toString(2)).substr(-8).split('').map(num => {
      if(num == '0'){
        result += '000'
      } else {
        result += '111'
...
function decode(bits){
  let newBits = bits.split('');
  let result = '';
  let binary = '';
  
  while(newBits.length > 0){
    let b = newBits.splice(0,3).join('');
    if(b == '000'){
      binary += '0';
    } else if(b == '111'){
      binary += '1';
    } else {
      if(b.toString().split('').filter(x => x == '0').length > 1){
        binary += '0'
      } else {
        binary += '1';
      };
  ...
  while(newBinary.length > 0){
    result += String.fromCharCode(parseInt(newBinary.splice(0,8).join(''),2));
  };
