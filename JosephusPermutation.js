...
// circle and proceeded to kill one man every three, until one last man was left (and that it was supposed 
// to kill himself to end the act).

// Well, Josephus and another man were the last two and, as we now know every detail of the story, you may 
// have correctly guessed that they didn't exactly follow through the original idea.
...
function josephus(items, k) {
  ...
  let res = [];
  let indexCtr = 0;
  let lastIndexCtr = items.length - 1;
  let removerCtr = 0;
  let lastRemoverCtr = k - 1;

  while (items.length != 0) {

    if (removerCtr == lastRemoverCtr) {
      res.push(items.splice(indexCtr,1)[0])
      removerCtr = 0;
      lastIndexCtr -= 1;
    } else {
      removerCtr += 1;    
