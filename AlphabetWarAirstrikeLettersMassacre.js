...
//Task
//Write a function that accepts fight string consists of only small letters and * which means a 
//bomb drop place. Return who wins the fight after bombs are exploded. When the left side wins 
...
function alphabetWar(fight){
  ...
  let bombIndex = newFight.indexOf('*');
  let leftSide = {'w':4,'p':3,'b':2,'s':1};
  let rightSide = {'m':4,'q':3,'d':2,'z':1};
  let leftScore = 0;
  let rightScore = 0;
  
  while(newFight.indexOf('*') > -1){    
    newFight[bombIndex] = '';
    if(newFight[bombIndex - 1] && newFight[bombIndex - 1] != '*'){
      newFight[bombIndex - 1] = '';
    }; 
    if(newFight[bombIndex + 1]  && newFight[bombIndex + 1] != '*'){
      newFight[bombIndex + 1] = '';
    };
    bombIndex = newFight.indexOf('*');
  };
  
  newFight.map(v => {
    if(leftSide[v]){
      leftScore += parseInt(leftSide[v]);
    };
    if(rightSide[v]){
      rightScore += parseInt(rightSide[v]);
    };
  });
