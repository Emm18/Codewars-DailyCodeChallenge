...
function alphabetWar(battlefield) {
  let newBattlefield = battlefield.split('');
  let bombsCoordinate = [];
  let shelters = [];
  let safe = [];
  let result = "";

  if (newBattlefield.includes('#') == true) {

    while (newBattlefield.indexOf('[') != -1 || newBattlefield.indexOf(']') != -1) {
      let openBracket = newBattlefield.indexOf('[');
      let closeBracket = newBattlefield.indexOf(']');
      shelters.push([openBracket, closeBracket])
      newBattlefield[openBracket] = "!";
      newBattlefield[closeBracket] = "!";
    }

    while (newBattlefield.indexOf('#') != -1) {
      let bomb = newBattlefield.indexOf('#');
      bombsCoordinate.push(bomb);
      newBattlefield[bomb] = "$"
    }
    ...
    for (let i = 0; i <= shelters.length - 1; i++) {
      ...
      if (shelters[i - 1]) {
        firstIndex = shelters[i - 1][1];
      } else {
        firstIndex = 0;
      }

      if (shelters[i + 1]) {
        lastIndex = shelters[i + 1][0];
      } else {
        lastIndex = newBattlefield.length - 1;
      }

      bombCount += bombsCoordinate.filter(x => x < shelters[i][0] && firstIndex <= x).length;
      bombCount += bombsCoordinate.filter(x => x > shelters[i][1] && x <= lastIndex).length;

      if (bombCount < 2) {
        safe.push(shelters[i]);
      }
    }

    safe.map((x, index) => {
      for (let i = x[0] + 1; i <= x[1] - 1; i++) {
        result += newBattlefield[i];
      }
    });
