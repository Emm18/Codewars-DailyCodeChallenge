...
//differs from the others in evenness. Help Bob — to check his answers, he needs a program that among 
...
function iqTest(numbers){
  ...
  let evenNumbers = arrNumbers.filter(x => x % 2 == 0);
  let oddNumbers = arrNumbers.filter(x => x % 2 == 1);

  return evenNumbers.length == 1 ? arrNumbers.indexOf(evenNumbers[0]) + 1 : arrNumbers.indexOf(oddNumbers[0]) + 1;
}
