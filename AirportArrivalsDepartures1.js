...
var flapDisplay = function (lines, rotors) {
  let arrValues = 'ABCDEFGHIJKLMNOPQRSTUVWXYZ ?!@#&()|<>.:=-+*/0123456789'.split('');
  let result = [];
  ...
  for (let i = 0; i <= lines.length - 1; i++) {
    ...
    for (let i = 0; i <= line.length - 1; i++) {
      line[i] = arrValues.indexOf(line[i]);
    }

    while (rotor.length > 0) {
      let moveCount = parseInt(rotor.splice(0, 1));
      for (let i = initialRotor; i <= line.length - 1; i++) {
        line[i] = parseInt(line[i]) + moveCount;
        if (parseInt(line[i]) > 53) {
          while (parseInt(line[i]) > 53) {
            line[i] = parseInt(line[i]) - 54;
          }
        }
      }
      initialRotor++;
