...
//code is the capital letter of the book category. In the bookseller's stocklist each 
//code c is followed by a space and by a positive integer n (int n >= 0) which indicates 
...
//If L or M are empty return string is "" (Clojure and Racket should return an empty array/list 
//instead).
...
function stockList(listOfArt, listOfCat){
  ...
  listOfArt.map(val => {
    let cat = val.split('')[0];
    let catVal = parseInt(val.split(' ')[1]);
    newListOfArt.push([cat, catVal, val]);
  });
  
  let zeroCtr = 0;
  listOfCat.map(cat => {
    let sum = 0;
    newListOfArt.filter(x => x[0] == cat).map(y => {
      sum += y[1];
    });
  ...
  return zeroCtr == listOfCat.length ? '' : result.join(' - ');
};
