...
//In this Kata you will be given an array representing contestant ranks. You must eliminate contestant in series of rounds comparing consecutive 
...
const tourney = array => {
  ...
  while(newArray.length != 1){
    ...
    if(newArray.length % 2 != 0){
      next.push(newArray[newArray.length - 1]);
    };
    for(let i = 0; i <= newArray.length - 2; i += 2){
      if(newArray[i] > newArray[i + 1]){
        next.push(newArray[i]);
      } else {
        next.push(newArray[i + 1]);
      };
    };
    newArray = next;
    result.push(next);
