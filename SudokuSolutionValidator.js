...
// and returns true if it is a valid solution, or false otherwise. The cells of the sudoku board may also contain 0's, which 
...
function validSolution(board) {
  ...
  let startingRow = 0;
  let startingCol = 0;
  while (startingRow < rows.length - 1) {
    let tempCol = startingCol;
    while (tempCol <= rows[0].length - 1) {
      let tempArr = [];
      let tempRow = startingRow;
      let initialCol = tempCol;
      while (tempRow <= startingRow + 2) {
        for (let j = tempCol; j <= tempCol + 2; j++) {
          tempArr.push(board[tempRow][j]);
        }
        tempCol = initialCol;
        tempRow += 1;
      }
  ...
  //check rows, cols, and 3x3
  if(!checkOneToNine(rows) && !checkOneToNine(cols) && !checkOneToNine(box3x3)){
    return true;
  } else {
    return false;
  }
...
function checkOneToNine(arr) {
  let validator = [1, 2, 3, 4, 5, 6, 7, 8, 9];
  let foundInvalid = false;

  for (let i = 0; i <= arr.length - 1; i++) {
    if (!foundInvalid) {
      validator.map(num => {
        if (arr[i].filter(val => val == num).length > 1) {
          foundInvalid = true;
        }
      });
    } else {
