...
//Contrary to popular belief, fish actually reduce pond maintenance as they graze on string algae and bottom feed 
//from the pond floor. They also make very enjoyable pets, providing hours of natural entertainment.
//Driving
//Task
...
function fish(shoal){
  ...
  for(let i = 1; i <= 9; i++){
    let ctr = newShoal.filter(x => x == i).length;
    fishCounts.push(i * ctr);
  };
  
  while(true){
    if(finalSize <= 9){
      eaten += fishCounts[finalSize - 1];  
    };
