...
function howmuch(m, n) {
  ...
  for(let i = minimum; i <= maximum; i++){
    ...
    let equationCar = (i - 1)/9;
    
    if(Math.floor(equationBoat) - equationBoat == 0){
      boatPossiblePrices.push([equationBoat, (7 * equationBoat) + 2]);
    };
     
    if(Math.floor(equationCar) - equationCar == 0){
      carPossiblePrices.push([equationCar, (9 * equationCar) + 1]);
    };
  };
    
  boatPossiblePrices.map(boat => {
    carPossiblePrices.map(car => { 
      if(car[1] == boat[1]){
        result.push([`M: ${boat[1]}`, `B: ${boat[0]}`, `C: ${car[0]}`]);
      };
    });
  });
