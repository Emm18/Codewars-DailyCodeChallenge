...
//                                           +--------------+
//do you put the $ sign in, or not? Inevitably, some people will type a $ sign and others will leave it out. The instructions could be made clearer - but 
//that won't help those annoying people who never read instructions anyway.
...
function money_value(s){
  let removedSign = s.split(' ').join('').split('').filter(x => x != '$').join('');
  return removedSign > 0 || removedSign < 0 ? removedSign : 0.0;
};
