...
//You have been put in charge of administrating vaccinations for children in your local area. Write a function that will generate a list of vaccines 
//for each child presented for vaccination, based on the child's age and vaccination history, and the month of the year.

//The function takes three parameters: age, status and month
//The parameter 'age' will be given in weeks up to 16 weeks, and thereafter in months. You can assume that children presented will be scheduled for 
//vaccination (eg '16 weeks', '12 months' etc).
//The parameter 'status' indicates if the child has missed a scheduled vaccination, and the argument will be a string that says 'up-to-date', or a 
//scheduled stage (eg '8 weeks') that has been missed, in which case you need to add any missing shots to the list. Only one missed vaccination 
...
function vaccineList(age, status, month) {
  ...
  let vaccineNeeded = [];
  let missed = status != 'up-to-date' ? true : false;
  let seasonal = vaccines.fluVaccine.includes(month) ? true : false;
  
  for(let val in vaccines){
    if(vaccines[val].includes(age)){
      vaccineNeeded = [...vaccineNeeded, val];
    };
    
    if(missed){
      if(vaccines[val].includes(status) && !vaccineNeeded.includes(val)){
        vaccineNeeded = [...vaccineNeeded, val];
      };
    };
    
    if(seasonal){
      if(vaccines[val].includes(month) && !vaccineNeeded.includes(val)){
        vaccineNeeded = [...vaccineNeeded, 'offer ' + val];
      };
    };
  };
  
  return vaccineNeeded.sort();
};
